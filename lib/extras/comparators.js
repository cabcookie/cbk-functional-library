...
var stringComparator = function (key, order) {
    ...
    return function (str1, str2) { return fp_1.flow(array_functions_1.map(fp_1.get(key)), array_functions_1.map(string_functions_1.upperCase), compareValues, ascDesc(order))([str1, str2]); };
};
exports.stringComparator = stringComparator;
var dateComparator = function (key, order) {
    if (key === void 0) { key = ''; }
    if (order === void 0) { order = 'asc'; }
    return function (date1, date2) { return fp_1.flow(array_functions_1.map(fp_1.get(key)), compareDateValues, ascDesc(order))([date1, date2]); };
};
